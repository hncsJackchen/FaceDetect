...
import com.jack.facedetect.bean.FeatureInfo;
import com.jack.facedetect.facecompare.FaceCompareManager;
import com.jack.facedetect.mediacodec.MediaHelper;
import com.jack.facedetect.presenter.HandleFace;
import com.jack.facedetect.util.CameraMatrix;
import com.jack.facedetect.util.ConUtil;
import com.jack.facedetect.util.ICamera;
import com.jack.facedetect.util.MediaRecorderUtil;
import com.jack.facedetect.util.OpenGLDrawRect;
import com.jack.facedetect.util.OpenGLUtil;
import com.jack.facedetect.util.PointsMatrix;
import com.jack.facedetect.util.Screen;
import com.jack.facedetect.util.SensorEventUtil;
import com.jack.facedetect.view.DialogUtil;
...
public class OpenglActivity extends Activity implements PreviewCallback, Renderer, SurfaceTexture.OnFrameAvailableListener {
    ...
    private long time_AgeGender_end = 0;
    private String AttriButeStr = "";
    private int angle;
    private int rotation = angle;
    private int preRotation = rotation;

    private Facepp.Face[] compareFaces;

    private long detectGenderAgeTime;
    private final int DETECT_GENDER_INTERVAL = 1000;
    private long featureTime = 0;
    ...
    private HandleFace mHandleFace;
...
//        }, 2000);
    ...
    private void init() {
        ...
        facepp = new Facepp();
        mSensorEventUtil = new SensorEventUtil(this);
...
//                Log.e("xie","xie rect"+compareFaces[0].rect.top+"bottom"+compareFaces[0].rect.bottom+newestFeature);
        ...
        //打开设置
        ImageView mIvSettings = findViewById(R.id.settings);
        mIvSettings.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent();
                intent.setClass(OpenglActivity.this, FaceppActionActivity.class);
...
//        newMethodCall();
    ...
    public void onPreviewFrame(final byte[] imgData, final Camera camera) {
        ...
        final int orientation = mSensorEventUtil.getOrientation();
        if (orientation == 0)
            rotation = angle;
        ...
        //检测人脸
        final Facepp.Face[] faces;
        final long faceDetectStartTime = System.currentTimeMillis();
        faces = facepp.detect(imgData, width, height, Facepp.IMAGEMODE_NV21);
        final long detectAlgorithmTime = System.currentTimeMillis() - faceDetectStartTime;

        if (faces != null) {
            long actionMaticsTime = System.currentTimeMillis();
            ArrayList<ArrayList> pointsOpenGL = new ArrayList<ArrayList>();//人脸关键点在图片中的位置
            ArrayList<FloatBuffer> rectsOpenGL = new ArrayList<FloatBuffer>();//人脸在图片中的位置
            ...
            if (faces.length > 0) {
                ...
                for (Facepp.Face face : faces) {
                    ...
                    //获取关键点信息
                    if (is106Points) {
                        facepp.getLandmarkRaw(face, Facepp.FPP_GET_LANDMARK106);
                    } else {
                    ...
                    Log.d(TAG, "confidence=" + confidence + ",age=" + face.age);
                }
        ...
        mHandler.post(new Runnable() {//facepp 子线程
            @Override
            public void run() {
                if (faces != null) {
                    ...
                    runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            String logStr = "\ncameraWidth: " + mICamera.cameraWidth + "\ncameraHeight: "
                                    + mICamera.cameraHeight + "\nalgorithmTime: " + detectAlgorithmTime + "ms"
